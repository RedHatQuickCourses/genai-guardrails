...
* The `pip` CLI is used to install Python libraries. In this lab, you should have installed the `termcolor` library
...
. We then import all the validator classes and set up the `Guard` object. The default behavior of a validator that fails is to throw an exception (You can change this behavior by changing the `on_fail` attribute of the individual validators).
+
[source,python]
--
...
...
<3> Use the competitor check validator. Takes a list of strings (competitor names) as an argument
...
+
The script takes a while to run. It downloads the validators and the associated fine-tuned small models for the validators. The corresponding test methods should have triggered the appropriate validators and the exception messages identify why the input texts failed validation.
+
...
. Experiment by changing the text in the variable `llm_response` in the test methods and see if the validators are triggered.
